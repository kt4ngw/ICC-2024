...
import numpy as np
from src.algorithms.algorithm3 import Algorithm3
class NumWSamplingTrainer(BaseFederated):
    def __init__(self, options, dataset, clients_label, cpu_frequency, B, transmit_power, g_N0):
        ...
        super(NumWSamplingTrainer, self).__init__(options, dataset, clients_label, cpu_frequency, B, transmit_power, g_N0, model
                                             , self.optimizer)
        self.prob = Algorithm3(clients_label, dataset).getProba()  # 输入带宽
        print(self.prob)
    def train(self):
